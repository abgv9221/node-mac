    ...
    script: {
      ...
      value: config.script !== undefined ? require('path').resolve(config.script) : null
		...
		root: {
			...
			writable: true,
			configurable: false,
			value: config.runAsAgent ? process.env.HOME + '/Library/LaunchAgents' : '/Library/LaunchDaemons'
		},
		...
		logpath: {
			...
			writable: true,
			configurable: false,
			value: config.logpath || process.env.HOME + '/Library/Logs/'+(this.name || config.name || 'node-scripts')
		},
				...
				this.stop(function(){
					exec('rm '+me.plist,{},function(){
						...
						if (fs.existsSync(me.logpath)){
							if (me.logpath !== process.env.HOME + '/Library/Logs'){
